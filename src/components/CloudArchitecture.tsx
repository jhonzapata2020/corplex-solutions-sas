...
import { Cloud, ShieldCheck, Layers, Cpu, Database, Server, Activity, CheckCircle2, ArrowRight, Play, RefreshCw, Zap } from 'lucide-react';
...
export const CloudArchitecture: React.FC = () => {
  ...
  return (
    <section id="arquitectura-cloud" className="py-24 relative bg-slate-950 border-t border-slate-800/80">
      
      {/* Glow Effects */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[400px] bg-sky-500/5 blur-[150px] rounded-full pointer-events-none" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-cyan-500/10 border border-cyan-500/30 text-cyan-300 text-xs font-mono font-semibold mb-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 relative">
            ...
            <div className="flex items-center justify-between pb-6 mb-6 border-b border-slate-800">
              <div>
                <span className="text-xs font-mono text-slate-400 block">TOPOLOGÍA AWS DE PRODUCCIÓN</span>
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-xl bg-sky-500/20 hover:bg-sky-500/30 text-sky-300 border border-sky-500/40 text-xs font-mono font-semibold flex items-center gap-2 transition-all disabled:opacity-50"
              >
                <Activity className={`w-3.5 h-3.5 text-sky-400 ${isSimulatingTraffic ? 'animate-spin' : ''}`} />
            ...
            <div className="space-y-4 relative py-4">
              ...
              {isSimulatingTraffic && (
                <div className="absolute left-6 top-8 bottom-8 w-1 bg-gradient-to-b from-emerald-400 via-sky-400 to-purple-400 animate-pulse z-20 pointer-events-none rounded-full" />
              )}
                ...
                return (
                  <div
                    ...
                    className={`p-4 sm:p-5 rounded-2xl border cursor-pointer transition-all duration-300 flex items-center justify-between gap-4 ${
                      isSelected
                        ? 'bg-slate-900 border-sky-500/80 shadow-lg shadow-sky-500/10 ring-1 ring-sky-500/40 translate-x-1'
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-xs font-mono text-slate-500 w-5 text-right">0{index + 1}</span>
                        <div className={`p-3 rounded-xl border ${
                          isSelected ? 'bg-sky-500/20 border-sky-500/40' : 'bg-slate-800 border-slate-700'
                        }`}>
                    ...
                    <div className="hidden sm:flex flex-col items-end shrink-0">
                      <span className="text-xs font-mono font-bold text-sky-400">{node.metrics}</span>
          ...
          <div className="lg:col-span-5 glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 sticky top-28">
            
            <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-6">
              <span className="text-xs font-mono text-sky-400 font-semibold">INSPECTOR DE NODO COMPONENT</span>
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <span className="text-xs font-mono text-slate-400">{selectedNode.serviceType}</span>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 mb-6">
              <span className="text-xs text-slate-400 font-mono block mb-1">MÉTRICA CLAVE // PERFORMANCE</span>
              <span className="text-lg font-mono font-extrabold text-sky-400">{selectedNode.metrics}</span>
            </div>

            <h4 className="text-xs font-mono text-slate-400 uppercase tracking-wider mb-3">ESPECIFICACIONES TÉCNICAS</h4>
            ...
            <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 text-center">
              <p className="text-xs text-slate-400 mb-2">¿Necesitas una arquitectura cloud personalizada para tu entidad?</p>
              <a
