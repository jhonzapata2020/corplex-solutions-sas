  ...
  return (
    <section id="inicio" className="relative pt-32 pb-24 md:pt-40 md:pb-32 overflow-hidden bg-quantango-mesh">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Ambient Diffuse Light Glow Behind Headline (Teal Emerald) */}
        <div className="absolute -z-10 w-[500px] h-[300px] bg-emerald-500/10 blur-[120px] rounded-full pointer-events-none top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2" />

        {/* Editorial Top Composition */}
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto mb-16">
          
          {/* Superior Micro-Pill Badge */}
          <div className="inline-flex items-center gap-2 text-[11px] font-mono-tech font-medium text-emerald-300 bg-zinc-900/80 border border-emerald-500/30 px-4 py-1.5 rounded-full tracking-widest uppercase mb-6 backdrop-blur-xl">
            <Sparkles className="w-3.5 h-3.5 text-emerald-400 stroke-[1.75]" />
            <span>SOFTWARE, AI & CLOUD INFRASTRUCTURE 2026</span>
          </div>

          {/* High-Tech H1 Headline with Teal Emerald Accent */}
          ...
          {/* Subtitle */}
          <p className="font-tech text-base sm:text-lg text-zinc-300 max-w-3xl leading-relaxed mb-10 font-normal">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto mb-16 font-tech">
            {/* Primary Amber Accent Pill Button */}
            <button
              onClick={onOpenQuoteModal}
              className="w-full sm:w-auto bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 hover:text-amber-200 border border-amber-500/40 hover:border-amber-400/80 px-6 py-2.5 rounded-full font-medium text-sm transition-all duration-300 shadow-lg shadow-amber-950/30 flex items-center justify-center gap-2 group cursor-pointer"
            >
              <span>Cotizar Solución</span>
              <ArrowRight className="w-4 h-4 text-amber-400 group-hover:translate-x-1 transition-transform stroke-[2]" />
            </button>

            {/* Secondary Glass Ghost Button */}
            <a
              href="#cumplimiento"
              className="w-full sm:w-auto bg-zinc-900/60 hover:bg-zinc-800/80 text-zinc-300 hover:text-white border border-zinc-800 hover:border-zinc-700 px-6 py-2.5 rounded-full font-medium text-sm transition-all duration-300 backdrop-blur-xl flex items-center justify-center gap-2"
            >
        ...
        <div className="relative max-w-5xl mx-auto font-tech">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 energy-orb-glow rounded-full pointer-events-none flex items-center justify-center">
            <svg className="w-full h-full text-emerald-500/20 animate-spin-slow opacity-60" viewBox="0 0 200 200" fill="none">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
            ...
            <div className="glass-smoked-card p-6 rounded-3xl border border-zinc-800 text-center flex flex-col items-center justify-between group">
            ...
            <div className="glass-smoked-card p-6 rounded-3xl border border-zinc-800 text-center flex flex-col items-center justify-between group">
              <div className="p-3 rounded-2xl bg-teal-500/10 border border-teal-500/30 text-teal-400 mb-4">
                <Zap className="w-6 h-6 stroke-[1.75]" />
              </div>
              <div className="text-3xl font-black text-white font-mono-tech mb-1 group-hover:text-teal-300 transition-colors">
                &lt; 20ms Latencia
              </div>
              <div className="text-xs font-mono-tech text-teal-400 font-bold mb-2">
            ...
            <div className="glass-smoked-card p-6 rounded-3xl border border-zinc-800 text-center flex flex-col items-center justify-between group">
              <div className="p-3 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-400 mb-4">
                <Lock className="w-6 h-6 stroke-[1.75]" />
              </div>
              <div className="text-3xl font-black text-white font-mono-tech mb-1 group-hover:text-amber-300 transition-colors">
                RDS Cifrado
              </div>
              <div className="text-xs font-mono-tech text-amber-400 font-bold mb-2">
